...
- **Генерация из текста**: три пресета качества, семь соотношений сторон,
  277 стилей Fooocus, до восьми изображений за запуск.
...
Адрес языковой модели, системные промты AI буста и отчёт о видеопамяти.
Токен доступа только записывается — страница его никогда не показывает.
...
4. **скачивает веса модели** (`Qwen/Qwen-Image-2.1`, ~33 ГБ) в `Qwen-Image-2.1/`
   — сверяет файлы с индексом модели, докачивает оборванное, не трогает то,
   что уже на месте;
5. **спрашивает адрес и токен языковой модели** для AI буста и проверяет связь.
   Enter пропускает шаг: без неё работает всё, кроме AI буста и «Описать изображение»;
6. проверяет окружение.
