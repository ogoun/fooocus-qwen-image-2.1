...
MESSAGES: dict[str, tuple[str, str]] = {
    ...
    # {seeds} — готовая фраза «Сид: 5» или «Сиды: 5, 6» (см. seeds_phrase).
    "generation_done": ("Готово. {seeds}. {memory}", "Done. {seeds}. {memory}"),
    "seed_one": ("Сид: {seeds}", "Seed: {seeds}"),
    "seed_many": ("Сиды: {seeds}", "Seeds: {seeds}"),
    ...
    "pose_add_hint": (
        ...
        "Choose a photo of a person in the pose you want: the pose is recognised and put into the cell, "
        "and its tile is drawn afterwards",
    ),
    "pose_missing": ("Этой позы уже нет на диске", "This pose is no longer on disk"),
    "pose_placed": ("Поза — в ячейке {cell}", "Pose put into cell {cell}"),
    "pose_not_found": ("На фото не найден человек — нужен снимок, где человек виден целиком или почти целиком",
                       "No person found in the photo — use one where the person is fully or almost fully visible"),
    "pose_failed": ("Поза не распознана: {error}", "Could not recognise the pose: {error}"),
    "pose_added": ("Поза распознана и положена в ячейку; рисую для неё плитку…",
                   "Pose recognised and put into the cell; drawing its tile…"),
    "pose_tile_drawing": ("Рисую плитку позы", "Drawing the pose tile"),
    "pose_tile_failed": ("Плитка позы не нарисовалась: {error}. Поза сохранена и доступна по скелету",
                         "The pose tile was not drawn: {error}. The pose is saved and shown as its skeleton"),
    "pose_tile_done": ("Плитка готова — поза добавлена в библиотеку", "Tile ready — the pose is in the library"),
