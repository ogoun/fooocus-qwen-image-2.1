...
- **Генерация из текста**: три пресета качества и **Turbo** (дистиллят на 6
  шагов, в 2–4 раза быстрее), семь соотношений сторон, 277 стилей Fooocus, до
...
16–40. В 3.8 раза быстрее MiddleQuality при том же разрешении и сопоставимом
качестве; умеет генерацию и правку. Адаптер (1.3 ГБ) скачивается при первом
...
- **INT8** — [INT8-веса Unsloth](https://huggingface.co/unsloth/Qwen-Image-2.1-FP8)
  в режиме «только веса»: 6.8 ГиБ, примерно на 5 % медленнее; LPIPS к bf16 по
  замеру Unsloth — 0.064. Пригодится, когда память нужна референсам или
  другим программам.
...
До десяти изображений добавляются в секции «Референсы» на вкладке генерации;
под каждой миниатюрой — её тег. При двух и более референсах промт обязан
ссылаться на них тегами `<image1>`, `<image2>`, … в порядке добавления:
«первое фото» модель не понимает. При ровно одном референсе теги не нужны.
...
| Turbo, 1888×1280 | 27.0 с | 17.0 ГиБ |
| Turbo + SageAttention, 1888×1280 | 22.6 с | 16.9 ГиБ |
| Turbo + SageAttention на INT8, 1888×1280 | 23.9 с | **10.4 ГиБ** |
| Загрузка модели | 33.8 с | |

Время Turbo — вместе с перестановкой текстового энкодера под новый промт;
подробности — [docs/research/2026-09-24-uskorenie-turbo-sage-int8.md](docs/research/2026-09-24-uskorenie-turbo-sage-int8.md).
...
fooocus_qwen/
  ...
  ui/          вкладки Gradio, кисть маски (ui/painter/), раскладка, переводы
resources/     стили Fooocus и системные промты Qwen
