...
Если полосы одинаковы — виновато не то, что мы добавили. Если у стокового
их нет — виновата наша сборка.

**Оговорка, стоившая неверного вывода.** Оба плеча идут через
``diffusers``, и такое сравнение не может отличить свойство модели от
свойства самого ``diffusers``. Независимый контроль — ComfyUI: он показал,
что полос там нет вовсе. Вывод «полосы свойство модели», к которому этот
опыт поначалу подтолкнул, был неверен — виноват был тайлинг VAE
(docs/research/2026-09-22-polosy-ot-tajlinga-vae.md).

Вторая оговорка — о мерке. Считать высокочастотный остаток по верхней
трети кадра нельзя: там есть содержимое, и оно разбавляет сигнал вчетверо.
Мерить надо по самой гладкой полосе кадра, как делает ``vae_tile_size.py``.
